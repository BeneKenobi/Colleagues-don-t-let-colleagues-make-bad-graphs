...
highway_signs <- highway_signs %>% 
  group_by(Size, Material) %>% 
  mutate(Observation = as.factor(row_number())) %>% 
  ungroup()

highway_signs$SizeMaterialObservation <- paste(highway_signs$Size, paste("Material",highway_signs$Material, sep=""), paste("Observation",highway_signs$Observation, sep=""), sep = "_")
barmeadows <- highway_signs %>% 
  ggplot(aes(x = SizeMaterialObservation, y = Distance, fill=bene_colors[1])) +
  ...
  theme(
    ...
    plot.margin = margin(l = 0 + margin_spacer(highway_signs$SizeMaterialObservation), t = 4)
  )
...
#| fig-cap: "Better presentation of multi-factorial data"
#| warning: false

dotsformultifactor <- highway_signs %>% 
  ggplot(aes(x = Material, y = Distance)) +
  facet_wrap( ~ Size, scales = "free_x", strip.position = "bottom") +
  geom_point(aes(fill = Material), shape = 21, size = 4, color = "black",
             position = position_jitter(seed = 2363, width = 0.2)) +
  bene_palette_shifted +
  theme_classic()
